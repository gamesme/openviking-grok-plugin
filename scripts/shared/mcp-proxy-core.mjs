...
export function createOpenVikingMcpProxy({
  ...
  async function handleMessage(message) {
    ...
    if (message.method === "initialize") {
      ...
      // Header must carry the server-negotiated version, never the client's
      // requested one — upstreams reject versions they don't know with HTTP 400.
      sessionId = "";
    }
    ...
    try {
      ...
      if (message.method === "initialize") {
        const negotiated = result.messages.find((m) => m?.result?.protocolVersion)?.result?.protocolVersion;
